...
#define	CC_GEMDEL_SEND_BODY_SIZE				1024
...
// gem/del/
enum GemDelStatus {
	CC_GEMDEL_NODE_NONE = 0,
	CC_GEMDEL_NODE_XML,
	CC_GEMDEL_NODE_XML_CHILD,
	CC_GEMDEL_NODE_API_STATUS
};
...
// 関数定義
//------------------------------------------------
static void CC_GemDel_CreateUri(const T_CC_CMN_SMS_API_PRM *param, Char *uri);
static void CC_GemDel_CreateBody(const T_CC_CMN_SMS_API_PRM *parm, const SMGEMDEL *gem, Char *body);
static E_SC_RESULT CC_GemDel_AnalyzeHttpResp(const Char *body, E_CONTEXT_TYPE contextType, const SMGEMDEL *gem, SMCALOPT *opt, Char *apiStatus);
static E_SC_RESULT CC_GemDel_XmlParse(const Char* xml, T_CC_CMN_SMS_RESPONSE_INFO* resp_inf, const SMGEMDEL* gem, SMCALOPT *opt);
static void XMLCALL CC_GemDel_StartElement(void *userData, const char *name, const char **atts);
static void XMLCALL CC_GemDel_EndElement(void *userData, const char *name);
static void XMLCALL CC_GemDel_CharacterData(void *userData, const XML_Char *data, INT32 len);

/**
 * @brief GEM削除
 * @param [IN] smcal        SMCAL
 * @param [IN] parm         APIパラメータ
 * @param [OUT] gem         GEM削除情報
 ...
 */
E_SC_RESULT CC_GemDel_SendRecv(SMCAL *smcal,
							   ...
							   Char *apiStatus)
{
	...
	do {
		...
		*apiStatus = EOS;
		...
		data = (Char*)SCC_MALLOC(CC_GEMDEL_SEND_BODY_SIZE);
		if (NULL == data) {
		...
		// URI生成
		CC_GemDel_CreateUri(parm, uri);

		// body生成
		CC_GemDel_CreateBody(parm, gem, data);
		...
		// レスポンス解析
		ret = CC_GemDel_AnalyzeHttpResp(body, contextType, gem, &opt, apiStatus);
		if (e_SC_RESULT_SUCCESS != ret) {
			SCC_LOG_ErrorPrint(SC_TAG_CC, (Char*)"CC_GemDel_AnalyzeHttpResp error, " HERE);
			break;
		}
	} while (0);
 ...
 * @return 処理結果(E_SC_RESULT)
 */
void CC_GemDel_CreateUri(const T_CC_CMN_SMS_API_PRM *parm, Char *uri)
{
	sprintf((char*)uri,
			"%sgem/del/",
 ...
 * @return 処理結果(E_SC_RESULT)
 */
void CC_GemDel_CreateBody(const T_CC_CMN_SMS_API_PRM *parm,
						  const SMGEMDEL *gem,
						  Char *body)
{
	sprintf((char*)body,
 ...
 * @return 処理結果(E_SC_RESULT)
 */
E_SC_RESULT CC_GemDel_AnalyzeHttpResp(const Char *body,
									  ...
									  Char *apiStatus)
{
	...
	do {
		...
		ret = CC_GemDel_XmlParse((const Char*)body, &rsp_inf, gem, opt);
		if(e_SC_RESULT_SUCCESS != ret) {
			SCC_LOG_ErrorPrint(SC_TAG_CC, (Char*)"CC_GemDel_XmlParse error, " HERE);
			ret = e_SC_RESULT_SMS_API_ERR;
			break;
		}
		...
		if (!CHECK_API_STATUS(rsp_inf.apiSts)) {
			// 『GEM削除済エラー』は、エラーとしない
			if (0 != strcmp("SMSAPIE047", rsp_inf.apiSts)) {
 ...
 * @return 処理結果(E_SC_RESULT)
 */
E_SC_RESULT CC_GemDel_XmlParse(const Char* xml, T_CC_CMN_SMS_RESPONSE_INFO* resp_inf, const SMGEMDEL* gem, SMCALOPT *opt)
{
	E_SC_RESULT		ret = e_SC_RESULT_SUCCESS;
	GEMDEL_PARSER	gemregParser = {};
	...
	do {
		...
		XML_SetElementHandler(parser, CC_GemDel_StartElement, CC_GemDel_EndElement);
		XML_SetCharacterDataHandler(parser, CC_GemDel_CharacterData);
 ...
 */
void XMLCALL CC_GemDel_StartElement(void *userData, const char *name, const char **atts)
{
	GEMDEL_PARSER *parser = (GEMDEL_PARSER*)userData;
...
//	SCC_LOG_DebugPrint(SC_TAG_CC, SCC_LOG_START);
	...
	do {
		...
		if (0 == strcmp((char*)name, (char*)CC_CMN_XML_NODE_NAME_XML)) {
			// <xml>
			parser->state = CC_GEMDEL_NODE_XML;
		}

		// <xml>
		if (CC_GEMDEL_NODE_XML == parser->state) {
			if (0 == strcmp((char*)name, (char*)CC_CMN_XML_NODE_NAME_API_STATUS)) {
				// <api_status>
				parser->state = CC_GEMDEL_NODE_API_STATUS;
			} else if (0 != strcmp((char*)name, (char*)CC_CMN_XML_NODE_NAME_XML)) {
				parser->state = CC_GEMDEL_NODE_XML_CHILD;
			}
		}
	} while (0);
 ...
 * @param [IN] name     タグ名
 */
void XMLCALL CC_GemDel_EndElement(void *userData, const char *name)
{
	GEMDEL_PARSER *parser = (GEMDEL_PARSER*)userData;
...
//	SCC_LOG_DebugPrint(SC_TAG_CC, SCC_LOG_START);
	...
	do {
		...
		if (CC_GEMDEL_NODE_API_STATUS == parser->state) {
			// <api_status>
			strcpy((char*)parser->gemInfo.apiStatus, (char*)parser->buf);
			parser->state = CC_GEMDEL_NODE_XML;
		} else if (CC_GEMDEL_NODE_XML_CHILD == parser->state) {
			parser->state = CC_GEMDEL_NODE_XML;
		}
	} while (0);
 ...
 * @param [IN] len      データ長
 */
void XMLCALL CC_GemDel_CharacterData(void *userData, const XML_Char *data, INT32 len)
{
	GEMDEL_PARSER *parser = (GEMDEL_PARSER*)userData;
	INT32	bufLen = 0;
