...
#define	CC_MAPPINGREG_XML_NODE_MAPPING_ID				"mpng_id"

#define	CC_MAPPINGREG_XML_DATA_MAPPING_ID_SIZE			CC_CMN_MAPPING_ID_SIZE
...
// マッピング登録XMLパーサ
typedef struct _MAPPINGREG_PARSER {
	INT32			state;
	Char			*buf;
...
// mapping/reg
enum MappingRegStatus {
	CC_MAPPINGREG_NODE_NONE = 0,
	CC_MAPPINGREG_NODE_XML,
	CC_MAPPINGREG_NODE_XML_CHILD,
	CC_MAPPINGREG_NODE_STATUS,
	CC_MAPPINGREG_NODE_API_STATUS,
	CC_MAPPINGREG_NODE_MAPPING_ID
};
...
// 関数定義
//------------------------------------------------
static void CC_MappingReg_CreateUri(const T_CC_CMN_SMS_API_PRM *param, Char *uri);
static E_SC_RESULT CC_MappingReg_CreatePostParam(const T_CC_CMN_SMS_API_PRM *parm, const SMMAPPINGINFO *mapping, SMCALPOSTPARAM *postParam, INT32 *postParamNum, BODY_BUF *bodyBuf);
static E_SC_RESULT CC_MappingReg_AnalyzeHttpResp(const Char *body, E_CONTEXT_TYPE contextType, SMMAPPINGINFO *mapping, SMCALOPT *opt, Char *apiStatus);
static E_SC_RESULT CC_MappingReg_XmlParse(const Char* xml, T_CC_CMN_SMS_RESPONSE_INFO* resp_inf, SMMAPPINGINFO* mapping, SMCALOPT *opt);
static void XMLCALL CC_MappingReg_StartElement(void *userData, const char *name, const char **atts);
static void XMLCALL CC_MappingReg_EndElement(void *userData, const char *name);
static void XMLCALL CC_MappingReg_CharacterData(void *userData, const XML_Char *data, INT32 len);
 ...
 */
E_SC_RESULT CC_MappingReg_SendRecv(SMCAL *smcal,
								   ...
								   Char *apiStatus)
{
	...
	do {
		...
		*apiStatus = EOS;
		...
		// URI生成
		CC_MappingReg_CreateUri(parm, uri);

		// POSTパラメータ生成
		ret = CC_MappingReg_CreatePostParam(parm, mapping, postParam, &postParamNum, &bodyBuf);
		if (e_SC_RESULT_SUCCESS != ret) {
			SCC_LOG_ErrorPrint(SC_TAG_CC, (Char*)"CC_MappingReg_CreatePostParam error, " HERE);
			break;
		}
		...
		// レスポンス解析
		ret = CC_MappingReg_AnalyzeHttpResp(body, contextType, mapping, &opt, apiStatus);
		if (e_SC_RESULT_SUCCESS != ret) {
			SCC_LOG_ErrorPrint(SC_TAG_CC, (Char*)"CC_MappingReg_AnalyzeHttpResp error, " HERE);
			break;
		}
	} while (0);
 ...
 * @return 処理結果(E_SC_RESULT)
 */
void CC_MappingReg_CreateUri(const T_CC_CMN_SMS_API_PRM *parm, Char *uri)
{
	sprintf((char*)uri,
			"%smpng/reg/",
 ...
 * @return 処理結果(E_SC_RESULT)
 */
E_SC_RESULT CC_MappingReg_CreatePostParam(const T_CC_CMN_SMS_API_PRM *parm,
										  ...
										  BODY_BUF *bodyBuf)
{
	...
	do {
		...
		postParam[num].type = SMCALPOSTPARAM_DATATYPE_TEXT;
		postParam[num].name = "\"genre\"";
		sprintf(bodyBuf->genre, "%d", mapping->genre);
		postParam[num].len = strlen(bodyBuf->genre);
		postParam[num].data = bodyBuf->genre;
		num++;
		...
		if ((NULL != mapping->circleGuid) && (EOS != mapping->circleGuid[0])) {
			postParam[num].type = SMCALPOSTPARAM_DATATYPE_TEXT;
			postParam[num].name = "\"circle_guid\"";
			postParam[num].len = strlen(mapping->circleGuid);
			postParam[num].data = (Char*)mapping->circleGuid;
			num++;
		}
		...
		postParam[num].type = SMCALPOSTPARAM_DATATYPE_TEXT;
		postParam[num].name = "\"mpng_datetime\"";
		// 現在時刻取得
		time(&nowTime);
 ...
 * @return 処理結果(E_SC_RESULT)
 */
E_SC_RESULT CC_MappingReg_AnalyzeHttpResp(const Char *body,
										  ...
										  Char *apiStatus)
{
	...
	do {
		...
		ret = CC_MappingReg_XmlParse((const Char*)body, &rsp_inf, mapping, opt);
		if(e_SC_RESULT_SUCCESS != ret) {
			SCC_LOG_ErrorPrint(SC_TAG_CC, (Char*)"CC_MappingReg_XmlParse error, " HERE);
			ret = e_SC_RESULT_SMS_API_ERR;
			break;
		}
								   ...
								   SMCALOPT *opt)
{
	...
	do {
		// 初期化
		resp_inf->sts = 0;
		mappingregParser.buf = (Char*)SCC_MALLOC(CC_CMN_XML_BUF_SIZE + 1);
		if (NULL == mappingregParser.buf) {
			SCC_LOG_ErrorPrint(SC_TAG_CC, (Char*)"SCC_MALLOC error, " HERE);
			CB_Result = e_SC_RESULT_MALLOC_ERR;
			ret = CB_Result;
			break;
		}
		mappingregParser.mappingInfo.apiStatus = &resp_inf->apiSts[0];
		mappingregParser.mappingInfo.id = mapping->mappingId;
		...
		// コールバック関数設定
		XML_SetUserData(parser, &mappingregParser);
		XML_SetElementHandler(parser, CC_MappingReg_StartElement, CC_MappingReg_EndElement);
		XML_SetCharacterDataHandler(parser, CC_MappingReg_CharacterData);
 ...
 */
void XMLCALL CC_MappingReg_StartElement(void *userData, const char *name, const char **atts)
{
	MAPPINGREG_PARSER *parser = (MAPPINGREG_PARSER*)userData;
...
//	SCC_LOG_DebugPrint(SC_TAG_CC, SCC_LOG_START);
	...
	do {
		...
		if (0 == strcmp((char*)name, (char*)CC_CMN_XML_NODE_NAME_XML)) {
			// <xml>
			parser->state = CC_MAPPINGREG_NODE_XML;
		}

		// <xml>
		if (CC_MAPPINGREG_NODE_XML == parser->state) {
			if (0 == strcmp((char*)name, (char*)CC_CMN_XML_NODE_NAME_API_STATUS)) {
				// <api_status>
				parser->state = CC_MAPPINGREG_NODE_API_STATUS;
			} else if (0 == strcmp((char*)name, (char*)CC_MAPPINGREG_XML_NODE_MAPPING_ID)) {
				// <mpng_id>
				parser->state = CC_MAPPINGREG_NODE_MAPPING_ID;
			} else if (0 != strcmp((char*)name, (char*)CC_CMN_XML_NODE_NAME_XML)) {
				parser->state = CC_MAPPINGREG_NODE_XML_CHILD;
			}
		} else {
			// 上記以外
 ...
 * @param [IN] name     タグ名
 */
void XMLCALL CC_MappingReg_EndElement(void *userData, const char *name)
{
	MAPPINGREG_PARSER *parser = (MAPPINGREG_PARSER*)userData;
...
//	SCC_LOG_DebugPrint(SC_TAG_CC, SCC_LOG_START);
	...
	do {
		...
		if (CC_MAPPINGREG_NODE_API_STATUS == parser->state) {
			// <api_status>
			strcpy((char*)parser->mappingInfo.apiStatus, (char*)parser->buf);
			parser->state = CC_MAPPINGREG_NODE_XML;
		} else if (CC_MAPPINGREG_NODE_MAPPING_ID == parser->state) {
			// <mpng_id>
			strcpy((char*)parser->mappingInfo.id, (char*)parser->buf);
			parser->state = CC_MAPPINGREG_NODE_XML;
		} else if (CC_MAPPINGREG_NODE_XML_CHILD == parser->state) {
			 parser->state = CC_MAPPINGREG_NODE_XML;
		}
	} while (0);
 ...
 * @param [IN] len      データ長
 */
void XMLCALL CC_MappingReg_CharacterData(void *userData, const XML_Char *data, INT32 len)
{
	MAPPINGREG_PARSER *parser = (MAPPINGREG_PARSER*)userData;
	INT32	bufLen = 0;
...
//	SCC_LOG_DebugPrint(SC_TAG_CC, SCC_LOG_START);
	...
	do {
		...
		if (CC_MAPPINGREG_NODE_API_STATUS == parser->state) {
			...
			// <mpng_id>
			if (CC_MAPPINGREG_XML_DATA_MAPPING_ID_SIZE > (bufLen + len)) {
				memcpy(&parser->buf[bufLen], data, len);
				parser->buf[bufLen + len] = EOS;
			}
		}
