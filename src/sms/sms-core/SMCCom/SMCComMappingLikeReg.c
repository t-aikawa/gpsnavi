...
// 関数定義
//------------------------------------------------
static void CC_MappingLikeReg_CreateUri(const T_CC_CMN_SMS_API_PRM *param, Char *uri);
static void CC_MappingLikeReg_CreateBody(const T_CC_CMN_SMS_API_PRM *parm, const Char *mappingId, Char *body);
static E_SC_RESULT CC_MappingLikeReg_AnalyzeHttpResp(const Char *body, E_CONTEXT_TYPE contextType, LONG *likeCnt, SMCALOPT *opt, Char *apiStatus);
static E_SC_RESULT CC_MappingLikeReg_XmlParse(const Char* xml, T_CC_CMN_SMS_RESPONSE_INFO* resp_inf, LONG *likeCnt, SMCALOPT *opt);
static void XMLCALL CC_MappingLikeReg_StartElement(void *userData, const char *name, const char **atts);
static void XMLCALL CC_MappingLikeReg_EndElement(void *userData, const char *name);
static void XMLCALL CC_MappingLikeReg_CharacterData(void *userData, const XML_Char *data, INT32 len);

/**
 * @brief マッピングにLikeをつける
 ...
 */
E_SC_RESULT CC_MappingLikeReg_SendRecv(SMCAL *smcal,
									   ...
									   Char *apiStatus)
{
	...
	do {
		...
		*apiStatus = EOS;
		...
		// URI生成
		CC_MappingLikeReg_CreateUri(parm, uri);

		// body生成
		CC_MappingLikeReg_CreateBody(parm, mappingId, data);
		...
		// レスポンス解析
		ret = CC_MappingLikeReg_AnalyzeHttpResp(body, contextType, likeCnt, &opt, apiStatus);
		if (e_SC_RESULT_SUCCESS != ret) {
			SCC_LOG_ErrorPrint(SC_TAG_CC, (Char*)"CC_MappingLikeReg_AnalyzeHttpResp error, " HERE);
			break;
		}
	} while (0);
 ...
 * @return 処理結果(E_SC_RESULT)
 */
void CC_MappingLikeReg_CreateUri(const T_CC_CMN_SMS_API_PRM *parm, Char *uri)
{
	sprintf((char*)uri,
			"%smpnglike/reg/",
 ...
 * @param [OUT] body        body
 */
void CC_MappingLikeReg_CreateBody(const T_CC_CMN_SMS_API_PRM *parm,
								  const Char *mappingId,
								  Char *body)
{
	sprintf((char*)body,
 ...
 * @return 処理結果(E_SC_RESULT)
 */
E_SC_RESULT CC_MappingLikeReg_AnalyzeHttpResp(const Char *body,
											  ...
											  Char *apiStatus)
{
	...
	do {
		...
		ret = CC_MappingLikeReg_XmlParse((const Char*)body, &rsp_inf, likeCnt, opt);
		if(e_SC_RESULT_SUCCESS != ret) {
			SCC_LOG_ErrorPrint(SC_TAG_CC, (Char*)"CC_MappingLikeReg_XmlParse error, " HERE);
			ret = e_SC_RESULT_SMS_API_ERR;
			break;
		}
...
/**
 * @brief mpnglike/reg/応答メッセージ解析
 * @param [IN] xml      XMLファイルのフルパス
 * @param [IN] resp_inf CICレスポンス情報
 * @param [OUT] likeCnt Like数
 * @return 処理結果(E_SC_RESULT)
 */
E_SC_RESULT CC_MappingLikeReg_XmlParse(const Char* xml,
									   ...
									   SMCALOPT *opt)
{
	...
	do {
		...
		XML_SetElementHandler(parser, CC_MappingLikeReg_StartElement, CC_MappingLikeReg_EndElement);
		XML_SetCharacterDataHandler(parser, CC_MappingLikeReg_CharacterData);
 ...
 */
void XMLCALL CC_MappingLikeReg_StartElement(void *userData, const char *name, const char **atts)
{
 ...
 * @param [IN] name     タグ名
 */
void XMLCALL CC_MappingLikeReg_EndElement(void *userData, const char *name)
{
	MAPPINGLIKE_PARSER *parser = (MAPPINGLIKE_PARSER*)userData;
 ...
 * @param [IN] len      データ長
 */
void XMLCALL CC_MappingLikeReg_CharacterData(void *userData, const XML_Char *data, INT32 len)
{
	MAPPINGLIKE_PARSER *parser = (MAPPINGLIKE_PARSER*)userData;
	INT32	bufLen = 0;
