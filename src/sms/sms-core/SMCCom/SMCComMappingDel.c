...
#define	CC_MAPPINGDEL_SEND_BODY_SIZE				1024
...
// マッピング削除XMLパーサ
typedef struct _MAPPINGDEL_PARSER {
	INT32			state;
	Char			*buf;
...
// mpng/del/
enum MappingDelStatus {
	CC_MAPPINGDEL_NODE_NONE = 0,
	CC_MAPPINGDEL_NODE_XML,
	CC_MAPPINGDEL_NODE_XML_CHILD,
	CC_MAPPINGDEL_NODE_API_STATUS
};
...
// 関数定義
//------------------------------------------------
static void CC_MappingDel_CreateUri(const T_CC_CMN_SMS_API_PRM *param, Char *uri);
static void CC_MappingDel_CreateBody(const T_CC_CMN_SMS_API_PRM *parm, const Char *mappingId, Char *body);
static E_SC_RESULT CC_MappingDel_AnalyzeHttpResp(const Char *body, E_CONTEXT_TYPE contextType, SMCALOPT *opt, Char *apiStatus);
static E_SC_RESULT CC_MappingDel_XmlParse(const Char* xml, T_CC_CMN_SMS_RESPONSE_INFO* resp_inf, SMCALOPT *opt);
static void XMLCALL CC_MappingDel_StartElement(void *userData, const char *name, const char **atts);
static void XMLCALL CC_MappingDel_EndElement(void *userData, const char *name);
static void XMLCALL CC_MappingDel_CharacterData(void *userData, const XML_Char *data, INT32 len);
 ...
 */
E_SC_RESULT CC_MappingDel_SendRecv(SMCAL *smcal,
								   ...
								   Char *apiStatus)
{
	...
	do {
		...
		*apiStatus = EOS;
		...
		data = (Char*)SCC_MALLOC(CC_MAPPINGDEL_SEND_BODY_SIZE);
		if (NULL == data) {
		...
		// URI生成
		CC_MappingDel_CreateUri(parm, uri);

		// body生成
		CC_MappingDel_CreateBody(parm, mappingId, data);
...
#endif
body = "<?xml version=\"1.0\" encoding=\"utf-8\"?>\n<xml><api_status>SMSAPII058</api_status><user_message>マッピングを削除しました。</user_message></xml>";

		// レスポンス解析
		ret = CC_MappingDel_AnalyzeHttpResp(body, contextType, &opt, apiStatus);
		if (e_SC_RESULT_SUCCESS != ret) {
			SCC_LOG_ErrorPrint(SC_TAG_CC, (Char*)"CC_MappingDel_AnalyzeHttpResp error, " HERE);
			break;
		}
	} while (0);
 ...
 * @return 処理結果(E_SC_RESULT)
 */
void CC_MappingDel_CreateUri(const T_CC_CMN_SMS_API_PRM *parm, Char *uri)
{
	sprintf((char*)uri,
			"%smpng/del/",
 ...
 * @return 処理結果(E_SC_RESULT)
 */
void CC_MappingDel_CreateBody(const T_CC_CMN_SMS_API_PRM *parm,
							  const Char *mappingId,
							  Char *body)
{
	sprintf((char*)body,
 ...
 * @return 処理結果(E_SC_RESULT)
 */
E_SC_RESULT CC_MappingDel_AnalyzeHttpResp(const Char *body,
										  ...
										  Char *apiStatus)
{
	...
	do {
		...
		ret = CC_MappingDel_XmlParse((const Char*)body, &rsp_inf, opt);
		if(e_SC_RESULT_SUCCESS != ret) {
			SCC_LOG_ErrorPrint(SC_TAG_CC, (Char*)"CC_MappingDel_XmlParse error, " HERE);
			ret = e_SC_RESULT_SMS_API_ERR;
			break;
		}
		...
		if ((!CHECK_API_STATUS(rsp_inf.apiSts)) && (!CHECK_API_STATUS2(rsp_inf.apiSts))) {
			// 『削除済エラー』は、エラーとしない
			if ((0 != strcmp("SMSAPIE103", rsp_inf.apiSts)) &&
				(0 != strcmp("SMSAPIE104", rsp_inf.apiSts))) {
...
/**
 * @brief mpng/del/応答メッセージ解析
 * @param [IN] xml      XMLファイルのフルパス
 * @param [IN] resp_inf CICレスポンス情報
 * @return 処理結果(E_SC_RESULT)
 */
E_SC_RESULT CC_MappingDel_XmlParse(const Char* xml, T_CC_CMN_SMS_RESPONSE_INFO* resp_inf, SMCALOPT *opt)
{
	E_SC_RESULT		ret = e_SC_RESULT_SUCCESS;
	MAPPINGDEL_PARSER	mappingParser = {};
	...
	do {
		// 初期化
		resp_inf->sts = 0;
		mappingParser.buf = (Char*)malloc(CC_CMN_XML_BUF_SIZE + 1);
		if (NULL == mappingParser.buf) {
			SCC_LOG_ErrorPrint(SC_TAG_CC, (Char*)"malloc error, " HERE);
			CB_Result = e_SC_RESULT_MALLOC_ERR;
			ret = CB_Result;
			break;
		}
		mappingParser.mappingInfo.apiStatus = &resp_inf->apiSts[0];
		...
		// コールバック関数設定
		XML_SetUserData(parser, &mappingParser);
		XML_SetElementHandler(parser, CC_MappingDel_StartElement, CC_MappingDel_EndElement);
		XML_SetCharacterDataHandler(parser, CC_MappingDel_CharacterData);
 ...
 */
void XMLCALL CC_MappingDel_StartElement(void *userData, const char *name, const char **atts)
{
...
//	SCC_LOG_DebugPrint(SC_TAG_CC, SCC_LOG_START);
	...
	do {
		...
		if (0 == strcmp((char*)name, (char*)CC_CMN_XML_NODE_NAME_XML)) {
			// <xml>
			parser->state = CC_MAPPINGDEL_NODE_XML;
		}

		// <xml>
		if (CC_MAPPINGDEL_NODE_XML == parser->state) {
			if (0 == strcmp((char*)name, (char*)CC_CMN_XML_NODE_NAME_API_STATUS)) {
				// <api_status>
				parser->state = CC_MAPPINGDEL_NODE_API_STATUS;
			} else if (0 != strcmp((char*)name, (char*)CC_CMN_XML_NODE_NAME_XML)) {
				parser->state = CC_MAPPINGDEL_NODE_XML_CHILD;
			}
		}
	} while (0);
 ...
 * @param [IN] name     タグ名
 */
void XMLCALL CC_MappingDel_EndElement(void *userData, const char *name)
{
	MAPPINGDEL_PARSER *parser = (MAPPINGDEL_PARSER*)userData;
...
//	SCC_LOG_DebugPrint(SC_TAG_CC, SCC_LOG_START);
	...
	do {
		...
		if (CC_MAPPINGDEL_NODE_API_STATUS == parser->state) {
			// <api_status>
			strcpy((char*)parser->mappingInfo.apiStatus, (char*)parser->buf);
			parser->state = CC_MAPPINGDEL_NODE_XML;
		} else if (CC_MAPPINGDEL_NODE_XML_CHILD == parser->state) {
			parser->state = CC_MAPPINGDEL_NODE_XML;
		}
	} while (0);
