...
#define CC_GEMLIKE_REG_SEND_BODY_SIZE	512
...
// 関数定義
//------------------------------------------------
static void CC_GemLikeReg_CreateUri(const T_CC_CMN_SMS_API_PRM *param, Char *uri);
static void CC_GemLikeReg_CreateBody(const T_CC_CMN_SMS_API_PRM *parm, const Char *gemId, Char *body);
static E_SC_RESULT CC_GemLikeReg_AnalyzeHttpResp(const Char *body, E_CONTEXT_TYPE contextType, LONG *likeCnt, SMCALOPT *opt, Char *apiStatus);
static E_SC_RESULT CC_GemLikeReg_XmlParse(const Char* xml, T_CC_CMN_SMS_RESPONSE_INFO* resp_inf, LONG *likeCnt, SMCALOPT *opt);
static void XMLCALL CC_GemLikeReg_StartElement(void *userData, const char *name, const char **atts);
static void XMLCALL CC_GemLikeReg_EndElement(void *userData, const char *name);
static void XMLCALL CC_GemLikeReg_CharacterData(void *userData, const XML_Char *data, INT32 len);
 ...
 */
E_SC_RESULT CC_GemLikeReg_SendRecv(SMCAL *smcal,
								   ...
								   Char *apiStatus)
{
	...
	do {
		...
		*apiStatus = EOS;
		...
		data = (Char*)SCC_MALLOC(CC_GEMLIKE_REG_SEND_BODY_SIZE);
		if (NULL == data) {
			SCC_LOG_ErrorPrint(SC_TAG_CC, (Char*)"SCC_MALLOC error, " HERE);
			ret = e_SC_RESULT_MALLOC_ERR;
			break;
		...
		// URI生成
		CC_GemLikeReg_CreateUri(parm, uri);

		// body生成
		CC_GemLikeReg_CreateBody(parm, gemId, data);
		...
		// レスポンス解析
		ret = CC_GemLikeReg_AnalyzeHttpResp(body, contextType, likeCnt, &opt, apiStatus);
		if (e_SC_RESULT_SUCCESS != ret) {
			SCC_LOG_ErrorPrint(SC_TAG_CC, (Char*)"CC_GemLikeReg_AnalyzeHttpResp error, " HERE);
			break;
		}
	} while (0);
 ...
 * @return 処理結果(E_SC_RESULT)
 */
void CC_GemLikeReg_CreateUri(const T_CC_CMN_SMS_API_PRM *parm, Char *uri)
{
	sprintf((char*)uri,
			"%s\?method=Gemlike.reg",
 ...
 * @param [OUT] body        body
 */
void CC_GemLikeReg_CreateBody(const T_CC_CMN_SMS_API_PRM *parm,
							  const Char *gemId,
							  Char *body)
{
	sprintf((char*)body,
 ...
 * @return 処理結果(E_SC_RESULT)
 */
E_SC_RESULT CC_GemLikeReg_AnalyzeHttpResp(const Char *body,
										  ...
										  Char *apiStatus)
{
	...
	do {
		...
		ret = CC_GemLikeReg_XmlParse((const Char*)body, &rsp_inf, likeCnt, opt);
		if(e_SC_RESULT_SUCCESS != ret) {
			SCC_LOG_ErrorPrint(SC_TAG_CC, (Char*)"CC_GemLikeReg_XmlParse error, " HERE);
			ret = e_SC_RESULT_SMS_API_ERR;
			break;
		}
		...
		if (!CHECK_API_STATUS(rsp_inf.apiSts)) {
			// 『LIKE重複エラー』は、エラーとしない
			if (0 != strcmp("SMSAPIE028", rsp_inf.apiSts)) {
 ...
 * @return 処理結果(E_SC_RESULT)
 */
E_SC_RESULT CC_GemLikeReg_XmlParse(const Char* xml,
								   ...
								   SMCALOPT *opt)
{
	...
	do {
		...
		XML_SetElementHandler(parser, CC_GemLikeReg_StartElement, CC_GemLikeReg_EndElement);
		XML_SetCharacterDataHandler(parser, CC_GemLikeReg_CharacterData);
 ...
 */
void XMLCALL CC_GemLikeReg_StartElement(void *userData, const char *name, const char **atts)
{
 ...
 * @param [IN] len      データ長
 */
void XMLCALL CC_GemLikeReg_CharacterData(void *userData, const XML_Char *data, INT32 len)
{
	GEMLIKE_PARSER *parser = (GEMLIKE_PARSER*)userData;
	//char buf[CC_CMN_XML_PARSE_DATA_SIZE + 1] = {};
	INT32	bufLen = 0;
