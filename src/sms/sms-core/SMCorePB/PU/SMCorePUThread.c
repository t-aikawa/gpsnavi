...
void *SC_PU_ThreadMain(void *param)
{
	...
	while (true != SC_Thread_GetIsFinish()) {
		...
		// メッセージ受信
		rc = pthread_msq_msg_receive(SC_CORE_MSQID_PU, &rmsg);
		if (PTHREAD_MSQ_OK != rc) {
			SC_LOG_ErrorPrint(SC_TAG_PU, "pthread_msq_msg_receive err, " HERE);
			continue;
		}
		...
		if (true != SC_MSG_CHECK_MSG_ID(&rmsg)) {
			SC_LOG_ErrorPrint(SC_TAG_PU, "SC_MSG_CHECK_MSG_ID err, " HERE);
		}

		// メッセージディスパッチ
		SC_MSG_MsgDispatch(&rmsg, SC_CORE_MSQID_PU);
	}
