...
E_SC_RESULT SC_PM_Initialize()
{
	...
	ret = SC_PM_ProbeInfo_Initial();				// プローブ情報初期化処理
...
void *SC_PM_ThreadMain(void *param)
{
	...
	while (true != SC_Thread_GetIsFinish()) {
		...
		// メッセージ受信
		rc = pthread_msq_msg_receive(SC_CORE_MSQID_PM, &rmsg);
		if (PTHREAD_MSQ_OK != rc) {
			SC_LOG_ErrorPrint(SC_TAG_PM, "pthread_msq_msg_receive err, " HERE);
			continue;
		}
		...
		if (true != SC_MSG_CHECK_MSG_ID(&rmsg)) {
			SC_LOG_ErrorPrint(SC_TAG_PM, "SC_MSG_CHECK_MSG_ID err, " HERE);
		}

		// メッセージディスパッチ
		SC_MSG_MsgDispatch(&rmsg, SC_CORE_MSQID_PM);
	}
