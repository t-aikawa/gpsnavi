...
E_SC_RESULT SC_MP_Initialize()
{
	...
	ret = SC_DRAW_Initialize();
	if (e_SC_RESULT_SUCCESS != ret) {
		SC_LOG_ErrorPrint(SC_TAG_MP, (Char*)"SC_DRAW_Initialize error(0x%08x), " HERE, ret);
	}
...
E_SC_RESULT SC_MP_Finalize()
{
	...
	ret = SC_DRAW_Finalize();
	if (e_SC_RESULT_SUCCESS != ret) {
		SC_LOG_ErrorPrint(SC_TAG_MP, (Char*)"SC_DRAW_Finalize error(0x%08x), " HERE, ret);
	}
...
void *SC_MP_ThreadMain(void *param)
{
	...
	while (true != SC_Thread_GetIsFinish()) {
		...
		// メッセージ受信
		rc = pthread_msq_msg_receive(SC_CORE_MSQID_MP, &rmsg);
		if (PTHREAD_MSQ_OK != rc) {
			SC_LOG_ErrorPrint(SC_TAG_MP, "pthread_msq_msg_receive error(0x%08x), " HERE, rc);
			continue;
		}

		// 受信メッセージをログ出力
		SC_LOG_DebugPrint(SC_TAG_MP,
		...
		if (true != SC_MSG_CHECK_MSG_ID(&rmsg)) {
			SC_LOG_ErrorPrint(SC_TAG_MP, "recv msgid error\n");
		}

		// メッセージディスパッチ
		SC_MSG_MsgDispatch(&rmsg, SC_CORE_MSQID_MP);
	}
