...
void *SC_SDT_ThreadMain(void *param)
{
	...
	while (true != SC_Thread_GetIsFinish()) {
		...
		// メッセージ受信
		rc = pthread_msq_msg_receive(SC_CORE_MSQID_SDT, &msg);
		if (PTHREAD_MSQ_OK != rc) {
			SC_LOG_ErrorPrint(SC_TAG_SDT, "pthread_msq_msg_receive err, " HERE);
			continue;
		}
		...
		if (true != SC_MSG_CHECK_MSG_ID(&msg)) {
			SC_LOG_ErrorPrint(SC_TAG_SDT, "SC_MSG_CHECK_MSG_ID err, " HERE);
		}

		// メッセージディスパッチ
		SC_MSG_MsgDispatch(&msg, SC_CORE_MSQID_SDT);
	}
